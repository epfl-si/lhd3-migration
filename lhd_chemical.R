insertChemical <- function(con, formHistory) {
  ...
  dataToBeMigrated <- result %>%
    mutate(
      ...
      submission = paste0('{"data":{"chemical":{"haz_en":"', haz_en, '"},"status":"Default","delete":false,"level":', score, '}}')
    ) %>% 

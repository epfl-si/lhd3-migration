insertMagnetic <- function(con, formHistory, formChildHistory) {
  ...
  dataToBeMigratedForChild <- mag %>%
    ...
    mutate(
      submission = ifelse(is.na(line_place), '', paste0('{"data":{"line":"',line,'","position":"',ifelse(line_place=='f', 'on the floor', ifelse(line_place=='n', 'not drawn on the floor', 'on the magnet')),'","otherImpactedRooms":[',other_rooms,'],"status":"Default","delete":false,"comment":"',ifelse(is.null(comment), '', ifelse(is.na(comment), '', comment)),'"}}'))
    ) %>%
    filter(submission != '') %>%
